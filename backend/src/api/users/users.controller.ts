...
export const saveArticle = async (req: Request, res: Response) => {
  try {
    const result = await userService.saveArticle(req.user.id, req.params.articleId);
    res.status(result.status).json(result);
  } catch (error: any) {
    res.status(500).json({
      success: false,
...
export const unsaveArticle = async (req: Request, res: Response) => {
  try {
    const result = await userService.unsaveArticle(req.user.id, req.params.articleId);
    res.status(result.status).json(result);
  } catch (error: any) {
    res.status(500).json({
      success: false,
...
export const checkEnrollment = async (req: Request, res: Response) => {
  try {
    const result = await userService.checkEnrollment(req.user.id, req.params.courseId);
    res.status(result.status).json(result);
  } catch (error: any) {
    res.status(500).json({
      success: false,
