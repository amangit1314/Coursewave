...
router.post('/rollback', verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    const rollbackToken = await TokenService.rollbackToken(userId, currentTokenId);
...
router.post('/validate-security', verifyToken, async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        isValid,
        message: isValid ? 'Token security validation passed' : 'Token security validation failed',
      },
    });
  } catch (error: any) {
