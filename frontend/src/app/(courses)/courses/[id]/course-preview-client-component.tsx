      ...
      <div className="space-y-12 md:px-10">
        ...
        {course && (
          <div className="md:max-w-screen w-full md:grid md:grid-cols-3">
            <div className="col-span-2 p-6 md:pl-0 md:p-0 md:pr-10">
...
const CourseDetailsLeftSection = ({ course }: { course: Course }) => {
  return (
    <div className="text-red flex flex-col items-start justify-center space-y-6 overflow-x-hidden text-start text-xl">
        ...
        <div className="w-full max-w-[45.313rem] space-y-4 rounded-3xl bg-gradient-to-br from-[#4144ff] to-[#57ffcf] py-4 pl-4 pr-4 backdrop-blur-sm md:mr-0 md:pr-0">
          ...
          {/* <div className="align-center backdrop-filter-blur flex w-auto flex-shrink-0 cursor-pointer flex-row items-center justify-start rounded-lg transition-all duration-200 md:items-start">
            <Image
              ...
              src={course?.instructor?.user[0].image || ""}
              alt={course?.instructor?.user[0].name || "Sample Instructor"}
              height={12}
              width={12}
              unoptimized
            />
      ...
      <Prerequisits prerequisits={course?.prerequisites} />
    </div>
  );
};
...
const CourseInstructorInformationSection = ({
  ...
  const instructorId = instructor?.user[0].id || "undefined";
...
const CourseEnrollButton = ({
  ...
  useEffect(() => {
    user && user?.id ? fetchEnrolledCourses() : null;
  ...
  return (
    <div>
      {isLoading || !courseId || !user?.id ? (
        <p className="pt-2 text-center text-xs font-thin text-blue-600">
        Need to login first
      </p>
      ) : (
        <p></p>
      ...
      <div className="flex justify-between">
        <Button
          ...
          disabled={isLoading || !courseId || !user?.id}
          size="sm"
          color="blue"
...
const AddToCartButton = ({ course }: { course: Course }) => {
  ...
  return (
    <Button
      ...
      size="sm"
      color="blue"
      className={`m-2 mt-2 w-[26rem] rounded-md bg-blue-500 text-center text-sm font-semibold text-white hover:bg-blue-700 ${
        isInCart ? "bg-blue-600" : "bg-blue-500"
      }`}
    >
      {isInCart ? (
