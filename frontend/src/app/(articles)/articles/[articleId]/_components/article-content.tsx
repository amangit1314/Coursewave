...
export function ArticleContent({ article }: ArticleContentProps) {
  ...
  return (
    <article className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-800/50 overflow-hidden">
      {/* Hero Image */}
      ...
      <div className="px-6 sm:px-8 pb-8">
        <motion.div
          ...
          className="prose prose-lg max-w-none prose-slate dark:prose-invert prose-headings:text-slate-900 dark:prose-headings:text-white prose-p:text-slate-700 dark:prose-p:text-slate-300 prose-strong:text-slate-900 dark:prose-strong:text-white"
        >
          {contentBlocks.map((block) => {
            switch (block.type) {
              case 'h1':
                return (
                  <h1 key={block.key} className="text-3xl font-bold text-slate-900 dark:text-white mb-6 mt-8 first:mt-0">
                    {block.content}
                  </h1>
                );
              case 'h2':
                return (
                  <h2 key={block.key} className="text-2xl font-bold text-slate-900 dark:text-white mb-4 mt-8">
                    {block.content}
                  </h2>
                );
              case 'h3':
                return (
                  <h3 key={block.key} className="text-xl font-bold text-slate-900 dark:text-white mb-3 mt-6">
                    {block.content}
                  </h3>
                );
              case 'p':
                return (
                  <p key={block.key} className="text-lg leading-relaxed text-slate-700 dark:text-slate-300 mb-6">
                    {block.content}
                  </p>
                );
              case 'spacing':
