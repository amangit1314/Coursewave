...
export const GET = async (
  ...
  const { courseSectionNumber, courseSectionTitle, courseSectionDescription } =
    reqBody;
  ...
  try {
    if (!instructorId || !courseId) {
      console.log("Invalid Instructor/course Id");
      return NextResponse.json(
        { success: false, message: "Invalid Instructor/course Id" },
        { status: 401 },
      );
    }
    ...
    if (!courseSectionTitle || !courseSectionDescription) {
      console.log(
        "Course section title and description are required field ...",
      );
      return NextResponse.json(
        {
          success: false,
    ...
    if (!instructor) {
      console.log(
        `No instructor found with this instructorid: ${instructorId} ...`,
      );
      return NextResponse.json(
        {
          success: false,
    ...
    if (!course) {
      console.log(`No course found with this courseid: ${courseId} ...`);
      return NextResponse.json(
        {
          success: false,
          message: `No course found with this courseid: ${courseId} ...`,
        },
        { status: 404 },
      );
    }
    ...
    console.log(
      "ERROR IN instructor/id/dashboard/courses/courseId/sections :",
      error.message,
    );
    return NextResponse.json(
      {
...
export const POST = async (
  ...
  try {
    ...
    if (courseSection) {
      console.log(
        `A section with this sectionTitle: ${courseSectionTitle},  already exists ...`,
      );
      return NextResponse.json(
        {
          success: false,
