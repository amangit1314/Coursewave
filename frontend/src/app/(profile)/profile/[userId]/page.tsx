...
const SwitchToInstructorButton = () => {
  ...
  const setInstructor = async () => {
    try {
      const response = await axios.post(`/api/profile/${user?.id}/becomeInstructor`);
      console.log("User is now instructor ...");
      toast.success("You are now instructor ✔️ ");
      router.push(`/instructor/${user?.id}/analytics`);
    } catch (error: any) {
      if (error.response?.status === 402) {
        // User is already an instructor, redirect to instructor view
        router.push(`/instructor/${user?.id}/analytics`);
        return;
      }
      console.log(`Error in becoming instructor: ${error.message}`);
