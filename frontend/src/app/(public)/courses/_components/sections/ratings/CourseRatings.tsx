...
import getUserInfoById from "@/lib/helpers/get_user_info_by_id";
import { absoluteUrl } from "@/lib/utils/utils";
import { useQuery } from "@tanstack/react-query";

// Define Review type if not available from Prisma
interface Review {
  id: string;
  userId: string | null;
  rating: number;
...
const ReviewAnalysis = ({
  ...
  return (
    <div className="mb-4 mr-4 grid w-full max-w-md grid-cols-1 items-start space-y-4 rounded-2xl shadow-lg bg-white  dark:bg-zinc-900/90 p-6 md:p-8">
      <div>
        <p className="text-xl font-bold dark:text-white">
          <strong className="text-2xl">{avgStarRatings.toFixed(1)}</strong>/5.0
        </p>
        <p className="text-sm dark:text-gray-400">Based on {totalReviews} reviews</p>
...
const ReviewsMassionaryGrid = ({ reviews }: { reviews: Review[] }) => {
  ...
  // Fetch user data for reviews with valid userIds
  React.useEffect(() => {
    const fetchUserData = async () => {
      setIsLoading(true);
      const validReviews = reviews.filter(review => review.userId && review.userId !== null);
      
      try {
        for (const review of validReviews) {
          if (review.userId && !userData[review.userId]) {
            try {
              ...
              console.error(`Failed to fetch user data for ${review.userId}:`, error);
              setUserData(prev => ({
                ...prev,
                [review.userId!]: { user: { name: "Anonymous", profileImageUrl: null } }
              }));
            }
          }
        }
  ...
  }, [reviews]); // Only run when reviews change
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {reviews.map((review: Review) => {
              ...
              if (!review.userId) {
                return (
                  <Reviewcard
                    ...
                    authorImgUrl="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRsdFvyaADX8a3uk59a3k94LeX8WRJEqVy6BQ&usqp=CAU"
              ...
              const user = userData[review.userId];
