...
export const ChaptersForm = ({ courseId, sectionId }: ChaptersFormProps) => {
  // const instructorId = course.instructorId;
  ...
  const { data: chapters = [], isLoading } = useChapters(courseId, sectionId);

  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      await axios.post(
        // `api/instructor/${instructorId}/dashboard/courses/${courseId}/sections/${sectionId}/chapters`,
  ...
  const onReorder = async (updateData: { id: string; position: number }[]) => {
    try {
      ...
      await axios.put(
        // `api/instructor/${instructorId}/dashboard/courses/${courseId}/sections/${sectionId}/chapters/reorder`,
        `api/courses/${courseId}/sections/${sectionId}/chapters`,
        {
          list: updateData,
        }
  ...
  return (
    <div className="relative mt-2 rounded-2xl border border-none bg-slate-100 p-4 dark:bg-zinc-900">
      ...
      <div className="flex items-center justify-between font-medium not-italic">
        ...
        <Button
          onClick={() => toggleCreating(sectionId)}
          variant="outline"
          className={`${dmSans.className} rounded-full`}
        >
          {isCreating ? (
          ...
          {!isCreating && (
            <div>
              {isLoading ? (
                ...
                <>
                  {!chapters.length && <span>No chapters</span>}
                  <ChaptersList
                    items={chapters ?? []}
                    onReorder={onReorder}
                    courseId={courseId}
