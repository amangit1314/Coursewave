...
//           {categoriesArray.length > 0 && (
//             <div className="mt-4 flex w-full max-w-[408px] space-x-2 overflow-y-hidden overflow-x-scroll pb-2">
...
export const CategoryForm = ({ course }: CategoryFormProps) => {
  const router = useRouter();
  const [isEditing, setIsEditing] = useState(false);
  const [points, setPoints] = useState(Array.isArray(course?.categories) ? course.categories : []);
