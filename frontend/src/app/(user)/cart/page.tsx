...
const CartPage = () => {
  // Zustand cart state/actions
  const { cartItems, totalItems, totalPrice, removeFromCart, clearCart } =
    useCartStore();
  ...
  // Basic cart items array (from Zustand)
  const items = cartItems;

  // Calculate subtotal using coursePrice
  const subtotal = items.reduce(
    (sum, item) => sum + parseFloat(item.coursePrice),
    0
  );
  const discount = appliedCoupon
    ? (subtotal * appliedCoupon.discount) / 100
  ...
  // Helper for instructor name from CartItem object
  const getInstructorName = (item: (typeof items)[number]) =>
    item.courseInstructorName || "";

  // Loading state shimmer (Zustand is sync, so you may skip this unless loading external data)
  // Fake loading shimmer for demo
              ...
              >
                <CardContent className="p-6">
                  <div className="flex flex-col sm:flex-row gap-6">
                    <div className="relative flex-shrink-0 w-full sm:w-52 h-36 rounded-2xl overflow-hidden shadow-lg">
                      <Image
                        src={item.courseImageUrl || "/placeholder_course.jpg"}
                        alt={item.courseName || "Course"}
                        fill
                        className="object-cover transition-transform duration-500 group-hover:scale-110"
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"></div>
                      {/* Discount Badge, if you wish */}
                      {/* <Badge ... /> */}
                      <div className="absolute bottom-3 left-3 flex items-center gap-1 bg-black/70 backdrop-blur-sm rounded-full px-2 py-1">
                        <Award className="w-3 h-3 text-yellow-400" />
                        <span className="text-white text-xs font-semibold">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            onClick={() =>
                              handleToggleWishlist(
                                item.courseId,
                                false /* TODO: Add real status if needed */
                              )
                            }
                            className="h-9 w-9 rounded-full text-gray-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-all hover:scale-110"
                          >
                      ...
                      {/* Stats */}
                      <div className="flex flex-wrap gap-3 mb-4">
                        {/* Duration, Rating, ... custom fields as needed */}
                        {/* ... */}
                        <div className="flex items-center gap-1.5 bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 px-3 py-1.5 rounded-full border border-green-100 dark:border-green-900">
                          <TrendingUp className="h-3.5 w-3.5 text-green-600 dark:text-green-400" />
                          <span className="text-xs font-semibold text-green-700 dark:text-green-300">
