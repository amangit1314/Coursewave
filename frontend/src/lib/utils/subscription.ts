...
export const getInstructorSubscriptionPlan = async (
  instructorId: string
): Promise<UserSubscription | null> => {
  try {
    // Get the auth token from localStorage
    ...
    if (!authToken) {
      console.warn(
        "No authentication token found for instructor subscription fetch"
      );
      return null;
    }

    const response = await fetch(
      `${API_BASE_URL}/api/subscriptions/instructor`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch instructor subscription data: ${response.status}`
      );
    }
...
export const getSubscriptionPlans = async (): Promise<SubscriptionPlan[]> => {
  try {
    console.log(
      "Fetching subscription plans from:",
...
export const subscribeToPlan = async (
  ...
  try {
    const endpoint =
      userType === "USER" ? "/user/subscribe" : "/instructor/subscribe";
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/subscriptions${endpoint}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const cancelSubscription = async (
  ...
  try {
    const endpoint =
      userType === "USER" ? "/user/cancel" : "/instructor/cancel";
