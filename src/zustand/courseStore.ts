...
const useCourseStore = create<CourseState & CourseActions>()((set) => ({
    ...
    fetchCourseSectionChapters: async (courseId: string, sectionId: string) => {
        ...
        //     const response = await fetch(`api/courses/${courseId}/sections/${sectionId}/chapters`);

        //     if (!response.ok) {
        //         set({ loading: false, error: 'Failed to fetch course section chapters ...' })
        //     }
    ...
    fetchCourseAttachments: async (courseId: string) => {
        try {
            ...
            if (!response.ok) {
                set({ loading: false, error: 'Failed to fetch course attachments ...' })
            }
    ...
    fetchCourseEnrollements: async (courseId: string) => {
        try {
            ...
            if (!response.ok) {
                set({ loading: false, error: 'Failed to fetch course enrollments ...' })
            }
