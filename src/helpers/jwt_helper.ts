...
export const generateAccessToken = async (payload: any) => {
    try {
        const token = await new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(process.env.ACCESS_TOKEN_EXPIRATION!).sign(key);
        return token;
    } catch (error) {
        console.error(error);
    }
...
export const generateRefreshToken = async (payload: any) => {
    try {
        ...
        const refreshToken = await new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(process.env.REFRESH_TOKEN_EXPIRATION!).sign(refreshKey);
...
export const generateVerificationToken = async (payload: any) => {
    try {
        const verificationToken = await new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime(process.env.REFRESH_TOKEN_EXPIRATION!).sign(key);
