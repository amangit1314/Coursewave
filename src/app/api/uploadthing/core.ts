...
const handleAuth = async () => {
  ...
  if (!user ) {
    ...
    throw new Error(`ERROR: UNAUTHORIZED, No user found with this ${userId} ⚠...`);
  }
...
const handleAuthForInstructor = async () => {
  const token = cookies().get('token')?.value;
...
export const ourFileRouter = {
  imageUploader: f({ image: { maxFileSize: "4MB" } })
    .middleware(async () => await handleAuthForInstructor())
  ...
  courseAttachment: f(["text", "image", "video", "audio", "pdf"])
    .middleware(() => handleAuthForInstructor())
