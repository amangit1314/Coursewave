...
const ArticlesPage = () => {
  ...
  // Use React Query instead of Zustand for better caching and fewer API calls
  ...
  if (error) {
    return (
      <div className="max-w-7xl overflow-x-hidden px-10 py-16 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-2xl font-bold text-zinc-800 dark:text-white mb-2">Failed to load articles</p>
        <p className="text-base text-red-500 dark:text-red-400 mb-6">{error.message}</p>
        <Button onClick={() => refetch()} className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
      ...
      <div>
        {filtered && filtered.length ? (
          <div className="my-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {filtered.map((article: BlogWithComments) => (
              <div key={article.id}>
                <ArticleCard article={article as unknown as BlogArticle} />
              </div>
            ))}
          </div>
        ) : (
          <div className="my-16 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xl font-semibold text-zinc-700 dark:text-gray-200 mb-2">No articles found</p>
            <p className="text-base text-zinc-500 dark:text-gray-400 mb-4">Try a different search or check back later.</p>
