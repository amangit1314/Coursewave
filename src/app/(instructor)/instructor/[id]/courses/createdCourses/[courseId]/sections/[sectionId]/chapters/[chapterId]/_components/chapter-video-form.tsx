      ...
      {!isEditing &&
        (!initialData.videoUrl ? (
          <div className="flex items-center justify-center h-60 bg-slate-200 rounded-md">
            <Video className="h-10 w-10 text-slate-500 rounded-xl overflow-hidden" />
          </div>
        ) : (
          <div className="relative aspect-video mt-2">
            <MuxPlayer className="rounded-xl overflow-hidden" playbackId={initialData?.muxData?.playbackId || ""} />
          </div>
        ))}
      {isEditing && (
        <div>
