...
const formSchema = z.object({
  thisCourseIsForPoints: z.string().array(),
});

export const ThisCourseIsForForm = ({ course }: { course: Course }) => {
  const [isEditing, setIsEditing] = useState(false);
  const [points, setPoints] = useState(
    course?.thisCourseIsFor || thisCourseIsForPoints
  );
  ...
  return (
    <div className="mt-6 border bg-slate-100 dark:bg-zinc-700 rounded-2xl p-4">
      ...
      {!isEditing && (
        <>
          {thisCourseIsForPoints!.length === 0 && (
